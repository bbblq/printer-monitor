...
// Test all HP and Ricoh MP 2014 printers
const printers = [
    { name: 'HP MFP437 (人力)', ip: '192.168.22.252' },
    { name: 'HP MFP232 (财务)', ip: '192.168.23.77' },
    { name: 'HP MFP226 (财务)', ip: '192.168.21.126' },
...
async function testPrinter(name, ip) {
    ...
    return new Promise((resolve) => {
        ...
        session.get(['1.3.6.1.2.1.1.1.0'], (error, varbinds) => {
            ...
            console.log(`✅ Online - ${varbinds[0].value.toString().substring(0, 60)}...`);
                ...
                for (const [idx, row] of rows.entries()) {
                    ...
                    if (row.desc && row.level != null && row.max != null) {
                        ...
                        if (row.level === -3) {
                            ...
                            console.log(`  Percent: ${percent}% (someRemaining - INACCURATE)`);
